...
const currentConfigType = ref<ConfigType | null>(null)
...
async function fetchConfigTypes() {
  loading.value = true
  try {
    const { data } = await configApi.typeList({
    ...
    if (data.value) {
      configTypes.value = data.value.records
...
async function fetchConfigItems(configTypeId: string) {
  try {
    const { data } = await configApi.itemList(configTypeId, {
      pageNumber: itemPage.value,
      pageSize: itemPageSize.value,
    })
    if (data.value) {
      configItems.value = data.value.records
...
onMounted(() => {
  Promise.all([fetchConfigTypes(), fetchEnableStatus()])
})
...
function handleItemPageChange(newPage: number) {
  ...
  if (currentConfigType.value) {
    fetchConfigItems(currentConfigType.value.id)
  }
}
...
function handleEdit(config: ConfigType) {
  isEdit.value = true
  formData.value = {
    id: config.id,
    name: config.name,
...
async function handleDelete(id: string) {
  const confirmed = await confirm('删除参数类型', '确定要删除该参数类型吗？')
  if (!confirmed) return
  try {
    await configApi.typeDelete(id)
    showSuccess('删除成功')
    if (configTypes.value.length === 1 && page.value > 1) {
      page.value--
    }
    fetchConfigTypes()
    configItems.value = []
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await configApi.typeUpdate(formData.value.id, formData.value)
    } else {
      await configApi.typeCreate(formData.value)
    }
...
function handleViewItems(config: ConfigType) {
  currentConfigType.value = config
  isListView.value = false
  fetchConfigItems(config.id)
}

function handleBackToList() {
  isListView.value = true
  currentConfigType.value = null
  configItems.value = []
}
...
function handleAddItem() {
  ...
  itemFormData.value = {
    id: '',
    configTypeId: currentConfigType.value?.id || '',
    label: '',
    key: '',
    value: '',
    type: 'text',
...
function handleEditItem(item: ConfigItem) {
  isEdit.value = true
  itemFormData.value = {
    id: item.id,
...
async function handleDeleteItem(id: string) {
  const confirmed = await confirm('删除参数项', '确定要删除该参数项吗？')
  if (!confirmed) return
  try {
    await configApi.itemDelete(currentConfigType.value!.id, id)
    showSuccess('删除成功')
    if (configItems.value.length === 1 && itemPage.value > 1) {
      itemPage.value--
    }
    if (currentConfigType.value) {
      fetchConfigItems(currentConfigType.value.id)
...
async function handleSubmitItem() {
  if (!itemFormData.value.label || !itemFormData.value.key) {
    showError('请填写必填项')
    return
  }
  try {
    if (isEdit.value) {
      await configApi.itemUpdate(
        currentConfigType.value!.id,
        itemFormData.value.id,
        itemFormData.value,
      )
    } else {
      await configApi.itemCreate(currentConfigType.value!.id, itemFormData.value)
    }
    showSuccess(isEdit.value ? '更新成功' : '新增成功')
    showDialog.value = false
    if (currentConfigType.value) {
  ...
  <div class="p-6 space-y-4 animate-page-enter">
    <template v-if="isListView">
      ...
      <div class="bg-card rounded-xl border shadow-sm">
        ...
        <Table v-else>
          ...
          <TableBody>
            <TableRow v-for="config in configTypes" :key="config.id">
    ...
    <template v-else>
      ...
      <div class="bg-card rounded-xl border shadow-sm">
        <Table>
          ...
          <TableBody>
            <TableRow v-for="item in configItems" :key="item.id">
            ...
            <TableRow v-if="configItems.length === 0">
    ...
    <Dialog v-model:open="showDialog" v-if="isListView">
      <DialogContent class="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{{ isEdit ? '编辑参数' : '新增参数' }}</DialogTitle>
          <DialogDescription></DialogDescription>
        </DialogHeader>
        <div class="grid grid-cols-2 gap-4 py-4">
          <div class="space-y-2">
            <Label for="name">参数名称 <span class="text-destructive">*</span></Label>
            <Input id="name" v-model="formData.name" placeholder="请输入参数名称" />
          </div>
          <div class="space-y-2">
            <Label for="code">参数编码 <span class="text-destructive">*</span></Label>
            <Input id="code" v-model="formData.code" placeholder="请输入参数编码" />
          </div>
          <div class="space-y-2">
    ...
    <Dialog v-model:open="showDialog" v-else>
      <DialogContent class="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{{ isEdit ? '编辑参数项' : '新增参数项' }}</DialogTitle>
          <DialogDescription></DialogDescription>
        </DialogHeader>
        <div class="grid grid-cols-2 gap-4 py-4">
          <div class="space-y-2">
            <Label for="label">参数标签 <span class="text-destructive">*</span></Label>
            <Input id="label" v-model="itemFormData.label" placeholder="请输入参数标签" />
          </div>
          <div class="space-y-2">
            <Label for="key">参数键名 <span class="text-destructive">*</span></Label>
            <Input id="key" v-model="itemFormData.key" placeholder="请输入参数键名" />
          </div>
          <div class="space-y-2">
            <Label for="value">参数值</Label>
            <Input id="value" v-model="itemFormData.value" placeholder="请输入参数值" />
          </div>
          <div class="space-y-2">
            <Label for="type">类型</Label>
            <DictSelect id="type" v-model="itemFormData.type" dict-type="config_type" />
          </div>
          <div class="space-y-2">
